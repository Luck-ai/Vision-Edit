    ...
    public static class ImageEffects
    {
            ...
            float rotationAngle)
        {
            ...
            if (contourThickness > 0)
            {
                ...
                // sticker is BGRA — without an explicit alpha the border is drawn fully transparent.
                var opaqueBorder = new MCvScalar(borderColor.V0, borderColor.V1, borderColor.V2, 255);
                CvInvoke.DrawContours(sticker, contours, -1, opaqueBorder, contourThickness);
            }
        ...
        public static Mat CropToContent(Mat mat)
        {
            if (mat.NumberOfChannels != 4) return mat.Clone();

            using Mat alpha = new Mat();
            CvInvoke.ExtractChannel(mat, alpha, 3);

            using Mat alphaBin = new Mat();
            CvInvoke.Threshold(alpha, alphaBin, 1, 255, ThresholdType.Binary);

            using VectorOfVectorOfPoint contours = new VectorOfVectorOfPoint();
            using Mat hierarchy = new Mat();
            CvInvoke.FindContours(alphaBin, contours, hierarchy, RetrType.External, ChainApproxMethod.ChainApproxSimple);

            if (contours.Size == 0) return mat.Clone();

            System.Drawing.Rectangle rect = CvInvoke.BoundingRectangle(contours[0]);
            for (int i = 1; i < contours.Size; i++)
            {
                rect = System.Drawing.Rectangle.Union(rect, CvInvoke.BoundingRectangle(contours[i]));
            }

            if (rect.Width <= 0 || rect.Height <= 0) return mat.Clone();
